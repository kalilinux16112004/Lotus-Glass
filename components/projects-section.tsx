...
import { ArrowRight, MapPin, Loader2, Download, X } from "lucide-react";
    ...
    <div className={cn("relative overflow-hidden", className)}>
      ...
      <img
        ...
        className={cn(
          ...
          isLoaded ? "opacity-100 group-hover:scale-110" : "opacity-0 scale-105"
        )}
      />
    </div>
  );
...
export function ProjectsSection() {
  ...
  useEffect(() => {
    getProjects().then((dbProjects) => {
      setProjects(dbProjects.length === 0 ? DEFAULT_PROJECTS : dbProjects);
      setIsLoading(false);
    });
  }, []);
  ...
  const years = ["All", ...Array.from(new Set(projects.map((p) => (p as any).year).filter(Boolean)))];

  // Apply Filters
  const filteredProjects = useMemo(() => {
    return projects.filter((project) => {
      const matchCategory = activeCategory === "All" || project.type === activeCategory;
  ...
  // Apply Pagination
  const displayedProjects = filteredProjects.slice(0, currentPage * ITEMS_PER_PAGE);
  ...
  return (
    <section id="projects" className="relative py-24 lg:py-32 bg-background overflow-hidden min-h-screen">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {displayedProjects.map((project, index) => (
                <div
                  ...
                  style={{ transitionDelay: isVisible ? `${(index % ITEMS_PER_PAGE) * 100}ms` : "0ms" }}
                >
                  <ProjectImage 
                    src={project.image || ""} 
                    alt={project.title} 
                    ...
                    <div className="rounded-xl border border-white/10 bg-white/5 backdrop-blur-md p-4 transition-all duration-500 group-hover:bg-white/10 group-hover:border-white/20 group-hover:-translate-y-1">
                      ...
                      <div className="flex justify-between items-center">
                        <div className="inline-flex items-center rounded-full border border-white/10 bg-white/10 px-3 py-1">
                          <span className="text-xs font-medium text-white/80">{project.type}</span>
                        </div>
            ...
            <div className="p-6 sm:p-10">
              ...
              <div className="prose prose-sm sm:prose-base dark:prose-invert max-w-none text-muted-foreground">
                <p>
                  ...
                  {/* @ts-ignore */}
                  {selectedProject.description || "Detailed information about this project is currently being updated. It reflects our rigorous standard for quality and structural integrity."}
                </p>
              </div>
            </div>
          </div>
